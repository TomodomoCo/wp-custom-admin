...
<h2><?php comment_form_title( _e('Leave a reply')); ?></h2>
...
<p> <a href="<?php echo wp_login_url( get_permalink() ); ?>"><?php _e('You must be logged in to post a comment.');?></a> </p>
<?php else : ?>
...
<p><?php _e('Logged in as:').' '?><a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php _e('Log out');?>"><?php _e('Log out');?></a></p>
...
<p>
<label for="email"><small><?php _e('Email'); _e('(will not be published)'); if ($req) _e('(required)'); ?></small></label>
